...
export async function POST(request: NextRequest) {
  ...
  const allowedIds = ids.filter((id) => {
    const row = db.prepare("SELECT id FROM comments WHERE id = ? AND profile_id = ? AND space_id = ? LIMIT 1").get(
      id,
  ...
  const { deletedIds, missingIds } = deleteFromTable("comments", allowedIds, auth.membership.space.id);
